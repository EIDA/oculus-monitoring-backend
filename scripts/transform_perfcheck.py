...
def send_to_zabbix(data_points, dry_run=False):
    ...
    if dry_run:
        ...
        for i, point in enumerate(zabbix_data[:5]):  # show firsts 5 points
            print(
                f" {i + 1}. host: {point['host']}, key {point['key']}, value: {point['value']}, timestamp: {point['clock']}"
            )
    ...
    for i in range(0, len(zabbix_data), chunk_size):
        ...
        try:
            ...
            if "result" in result:
                print(
                    f"successfully sent chunk {chunk_num}/{total_chunks} ({len(chunk)} items)"
                )
            else:
                print(
                    f"error sending chunk {chunk_num}: {result.get('error', 'unknow error')}"
                )
                return False
    ...
    try:
        ...
        for item in data:
            ...
            transfer_rate = calculate_transfer_rate(
                item["responselength"], item["requestduration"]
            )
        ...
        if filtered_items:
            ...
            for item in filtered_items:
                print(
                    f"    - {item['original_testid']} (host: {item['extracted_host']}) - {item['reason']}"
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="transform perfcheck results and send to zabbix"
    )
    parser.add_argument(
        "--input",
        "-i",
        default="pc_tests_results_clean.json",
        help="input json file default: pc_tests_results_clean.json",
    )
    parser.add_argument(
        "--output",
        "-o",
        default="pc_tests_results_transformed.json",
        help="output json file (default: pc_tests_results_transformed.json)",
    )
    parser.add_argument(
        "--send_file", "-s", help="send specific transformed file to zabbix"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
