...
def clone_repository():
    ...
    try:
        ...
        subprocess.run(
            ["git", "clone", repo_url, clone_dir], check=True, capture_output=True
        )

        nodes_dir = os.path.join(clone_dir, "eida_nodes")

        if not os.path.exists(nodes_dir):
            log_and_print("eida_nodes directory not found, in cloned repository")
            return None
    ...
    try:
        ...
        for key, metrics in results.items():
            ...
            items.extend(
                [
                    ItemValue(
                        hostname, f"{key}.status_code", str(metrics["status_code"])
                    ),
                    ItemValue(
                        hostname, f"{key}.response_time_ms", metrics["response_time_ms"]
                    ),
                    ItemValue(
                        hostname,
            ...
            log_and_print(
                f"    {key}.content_size_bytes = {metrics['content_size_bytes']}"
            )
        ...
        log_and_print(
            f"{hostname}: {response.processed}/{response.total} items send successfully"
        )
...
def process_node(node_name, node_data):
    ...
    for check in perf_checks:
        ...
        log_and_print(
            f" -> status: {result['status_code']}, time: {result['response_time_ms']}ms, size: {result['content_size_bytes']} bytes"
        )
    ...
    for node_name, node_data in yaml_data.items():
        ...
        if results:
            ...
            if send_to_zabbix(hostname, results):
                ...
                log_and_print(
                    f"{node_name}: perfCheck completed but zabbix sending failed"
                )
        else:
